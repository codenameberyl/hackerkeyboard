...
- **The API 33 (Android 13) CI emulator job is flaky in this environment.** Its emulator image timed out waiting to boot twice (once at a 600s timeout, once even after raising that to 900s) before finally booting and passing on a third attempt, while API 34/35 boot reliably in ~8 minutes every time under an identical config. It's marked non-blocking (`continue-on-error`) as a result: it still runs and its results are visible, but a slow boot on that specific image no longer fails the whole workflow. API 34/35 passing consistently is good evidence the app itself, the test setup, and the emulator infrastructure are all sound -- this is specifically about API 33's boot time being on the edge of (and once past) the timeout.

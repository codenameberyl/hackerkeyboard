...
- **Home screen rebuilt again, from the actual reference app's source
  this time.** The two passes above were built from an HTML/CSS mockup
  and a hand-typed single-file XML reconstruction of the same design,
  both generated by a different chat session rather than read from the
  real thing -- and both turned out to have drifted from it in ways
  that mattered (an invented yellow accent color where the real app
  has none, wrong letter-spacing/badge/button sizes, full-width buttons
  where the real ones are compact, a custom-bordered `EditText` where
  the real test field is a proper Material `TextInputLayout`). Cloned
  [jcb1ee/hackerskeyboard2](https://github.com/jcb1ee/hackerskeyboard2)
  directly and read its actual `main.xml`/`themes.xml`/`colors.xml`
  instead of continuing to guess:
  - The real app's Home screen isn't black+yellow at all -- it's a
    proper day/night-adaptive navy/cream scheme (`Theme.HackersKeyboard.Main`
    there): cream `#F0EBD8` background / white cards / near-black text
    in light mode, navy `#0D1321` background / lighter-navy `#1D2D44`
    cards / cream text in dark mode, with a consistent navy-or-slate
    accent for badges and buttons -- no yellow anywhere. Added a new
    `Theme.HackersKeyboard.Main` (own `setup_*` colors in
    `colors.xml`/`values-night/colors.xml`, exact values from the
    source) applied via the manifest's per-activity `android:theme`,
    scoped to `Main` only -- the rest of the app (Settings, panels)
    keeps its existing fixed black/yellow `Theme.HackersKeyboard`
    untouched, since this request was specifically about the Home
    screen. `Theme.HackersKeyboard.Main` is a `NoActionBar` variant
    (matching the source), so `Main.java`'s `getSupportActionBar().hide()`
    workaround from the earlier pass is now unnecessary and removed.
  - Header icon: a real `ImageView` showing the actual app icon
    (`@drawable/icon`) inside a padded, bordered box, replacing the
    invented "Esc" keycap-in-socket text treatment from the earlier
    passes (the real app doesn't have one).
  - Section labels ("SETUP"/"TRY IT OUT"): 11sp with 0.1 letter-spacing
    in the muted secondary color, not the invented primary-accent
    yellow at 12.5sp/0.06 from the previous pass.
  - Cards: 12dp corner radius (was 16dp), 16dp internal padding (was
    20dp), a uniform 10dp gap between all three (no special larger gap
    before Settings) -- the extra breathing room before the Settings
    button instead comes from the button's own 16dp `marginTop`, and
    before "TRY IT OUT" from that label's own 24dp `marginTop`, both
    matching the source's actual margin placement.
  - Step badges 32dp (was 38dp) with 14sp text; badge-to-text gap 12dp
    (was 16dp); step titles 16sp (was 17sp), descriptions 13sp (was
    13.5sp) with a 2dp top margin.
  - Buttons: `wrap_content` width (was full-width `match_parent`) for
    the three step buttons, 8dp corner radius (was 12dp), no forced
    `minHeight`/text size -- Material's own defaults, matching the
    source exactly instead of an invented 50dp/14.5sp. The Settings
    button drops the previous pass's explicit dimmed-yellow stroke
    override (that color doesn't exist in this palette); its outline
    now just follows the theme's own `colorOutline` like everything
    else.
  - The "try it out" field is now a real `TextInputLayout`
    (`OutlinedBox` style) + `TextInputEditText` with a floating hint,
    nested in a plain `MaterialCardView`, replacing the previous pass's
    custom-drawable-bordered `EditText` -- matching the source's actual
    component choice, including `imeOptions="flagNoFullscreen"` so
    testing the keyboard on this field doesn't make it go fullscreen.
  - Version text color is the muted/outline tone (matching the source
    reusing that same color for both borders and this text), not the
    previous pass's secondary-text color.
  The "Get dictionaries" button and the detailed HTML help text below
